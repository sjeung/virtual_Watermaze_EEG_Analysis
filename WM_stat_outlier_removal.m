...
for Pi = participantIDs
    try
        ...
        ERSPPS.powspctrm(matchVec(outInds == 1 & matchVec(:,1) == 3,2),:,:,:) = [];
        ERSPPE.powspctrm(matchVec(outInds == 1 & matchVec(:,1) == 4,2),:,:,:) = [];
        
        ERSPLS.cumtapcnt(matchVec(outInds == 1 & matchVec(:,1) == 1,2),:) = [];
        ERSPLE.cumtapcnt(matchVec(outInds == 1 & matchVec(:,1) == 2,2),:) = [];
        ERSPPS.cumtapcnt(matchVec(outInds == 1 & matchVec(:,1) == 3,2),:) = [];
        ERSPPE.cumtapcnt(matchVec(outInds == 1 & matchVec(:,1) == 4,2),:) = [];
        
        ERSPLS.trialinfo(matchVec(outInds == 1 & matchVec(:,1) == 1,2)) = [];
        ERSPLE.trialinfo(matchVec(outInds == 1 & matchVec(:,1) == 2,2)) = [];
        ERSPPS.trialinfo(matchVec(outInds == 1 & matchVec(:,1) == 3,2)) = [];
        ERSPPE.trialinfo(matchVec(outInds == 1 & matchVec(:,1) == 4,2)) = [];
